...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            var message = this.attributes.speechOutput = this.t('WELCOME_MESSAGE', this.t('SKILL_NAME'));
            var reprompt = this.attributes.speechOutput = this.t('WELCOME_REPROMT');
        ...
        'getNews' : function () {
            ...
            this.emit(':tell', "<p>Here\'s the latest <say-as interpret-as='characters'>ABC</say-as> news.</p>");
            controller.play.call(this);
        },
        'getNewsRead': function(){
            const itemSlot = this.event.request.intent.slots.news_categoryslot;
            let itemName;
            if (itemSlot && itemSlot.value) {
                itemName = itemSlot.value.toLowerCase();
            }
            const myCategories = this.t('CATEGORY');
            const category = myCategories[itemName];

            if (category) {
                /* Resetting audio to not play*/
                this.attributes.speechOutput = category;
                this.emit(':tell', category);
            } else {
                let speechOutput = this.t('CATEGORY_NOT_FOUND_MESSAGE');
                const repromptSpeech = this.t('CATEGORY_NOT_FOUND_REPROMPT');
                if (itemName) {
                    speechOutput += this.t('CATEGORY_NOT_FOUND_WITH_ITEM_NAME', itemName);
                } else {
                    speechOutput += this.t('CATEGORY_NOT_FOUND_WITHOUT_ITEM_NAME');
                }
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'Welcome to ABC news. You can say, get me the news.';
        ...
        'Unhandled' : function () {
            var message = 'Sorry, I could not understand. Please say, get me the news, to begin the audio.';
            this.response.speak(message).listen(message);
            this.emit(':responseReady');
        }
    }),
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
            ...
            if (this.attributes['playbackFinished']) {
                this.handler.state = constants.states.START_MODE;
                message = 'Welcome to ABC news. You can say, get me the news.';
                reprompt = 'You can say, get me the news, to begin.';
            } else {
        ...
        'getNews' : function () { controller.play.call(this) },
        'getNewsRead': function(){
            const itemSlot = this.event.request.intent.slots.news_categoryslot;
            let itemName;
            if (itemSlot && itemSlot.value) {
        ...
        'AMAZON.HelpIntent' : function () {
            ...
            var message = 'You are listening to the ABC news. You can say, Next or Previous to navigate through the playlist. ' +
        ...
        'Unhandled' : function () {
            var message = 'Sorry, I could not understand. You can say, get me the news.';
            this.response.speak(message).listen(message);
            this.emit(':responseReady');
        }
    }),
